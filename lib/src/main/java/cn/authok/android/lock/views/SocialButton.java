...
class SocialButton extends RelativeLayout {
    ...
    private void init() {
        inflate(getContext(), R.layout.cn_authok_lock_btn_social_large, this);
        icon = findViewById(R.id.cn_authok_lock_icon);
        title = findViewById(R.id.cn_authok_lock_text);
    ...
    private StateListDrawable getTouchFeedbackBackground(@ColorInt int pressedColor) {
        ...
        if (shouldDrawOutline) {
            int outlineColor = getResources().getColor(R.color.cn_authok_lock_social_light_background_focused);
    ...
    public void setStyle(AuthConfig config, @AuthMode int mode) {
        ...
        if (shouldUseDarkText) {
            int textColor = getResources().getColor(R.color.cn_authok_lock_social_text_light);
            title.setTextColor(textColor);
        }
        icon.setImageDrawable(logo);
        final String name = config.getName(getContext());
        final String prefixFormat = getResources().getString(mode == AuthMode.LOG_IN ? R.string.cn_authok_lock_social_log_in : R.string.cn_authok_lock_social_sign_up);
