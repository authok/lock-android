...
public class ClassicBuilderTest {
    ...
    public void shouldThrowIfAccountIsMissing() {
        expectedException.expect(IllegalStateException.class);
        expectedException.expectMessage("Missing Authok account information.");
    ...
    public void shouldCreateAccountFromResources() {
        ...
        Mockito.when(resources.getIdentifier(eq("cn_authok_client_id"), eq("string"), eq("test.app"))).thenReturn(1);
        Mockito.when(resources.getIdentifier(eq("cn_authok_domain"), eq("string"), eq("test.app"))).thenReturn(2);
